<template>
  <div class="mx-auto max-w-315 w-full sm:py-18 p-10 border-b border-[#EAECF0]">
    <div class="flex flex-col sm:flex-row justify-between">
      <div class="flex flex-col gap-8">
        ...
        <div class="sm:flex flex-col hidden sm:flex-row gap-6">
          <div v-for="(i, ix) in menuItems" :key="ix">
            <p class="text-black text-base">{{ i.label }}</p>
          </div>
        </div>
      </div>
  ...
  <div
    class="sm:py-10 sm:pr-10 p-10 flex flex-col sm:flex-row justify-between items-center"
  >
    <div>
...
<script setup>
const menuItems = [
  { label: 'Sobre a Most', to: '/' },
  { label: 'Soluções', to: '/' },
  { label: 'Parceiro', to: '/' },
  { label: 'Incorporadora', to: '/' },
